...
void instanceinitech(int samplingFreq) {
	int i = 0;
	...
	fslider1 = 0.0f;
	IOTA = 0;
	for (i=0; i<262144; i++) fRec0[i] = 0;
	fcheckbox0 = 1.0;
	fSamplingFreq = samplingFreq;
}

void computeech (int count, float** input, float** output) {
		int i;
...
process (jack_nframes_t nframes, void *arg)
{
   ...
   //fslider0 = 0.0f;
   //fslider1 = 0.0f;
   computeech(nframes, &in1, &out1);
   //fslider0 = 1500.0f;
   //fslider1 = 90.0f;
   //computeech(nframes, &in2, &out2);
...
*/
	...
	instanceinitech(44100);
	
	
   if (argc >= 2)
   {
      client_name = argv[1];
      if(argc >= 3) fslider0 = (float)atoi(argv[2]);
      if(argc >= 4) fslider1 = (float)atoi(argv[3]);
   }
   else
   {
      fprintf (stderr, "Usage : jack_simple_effect <client_name> <time:0~2000> <release:0~100>\n");
      exit (1);
   }
